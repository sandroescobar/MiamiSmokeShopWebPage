...
## Overview
- Full-stack Express + EJS storefront for Miami Vape Smoke Shop with manual checkout, MySQL catalog, and policy pages.
...
## Tech Stack
- Runtime: Node.js 20+ (ESM) with Express 5, MySQL2, dotenv, Nodemon.
...
- `index.js`: Main Express app configuring env parsing, MySQL pool, static assets, policy content, and JSON APIs (health, store list, inventory checks, closest-store lookup, checkout/cart flows).
...
## Local Development
1. Install Node deps: `npm install` (already includes Express, EJS, and MySQL clients).
2. Start the server: `npm run "start smoking"` (runs `node index.js`). Use `nodemon index.js` for auto-reload if desired.
3. Populate `.env` with database credentials and contact numbers, then ensure MySQL has required tables (`products`, `stores`, `categories`, etc.).
...
- Server logs (`logs.txt`) capture operational output; delivery + MySQL errors log to console.
